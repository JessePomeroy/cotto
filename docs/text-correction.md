...
Because `sorry` can introduce an apology and er/err/erm can precede a new thought, their deletion exceptions require an anchored single-word replacement, a direct quantity replacement, or repeated wording with a negation change. Broader lexical or clause restarts can conservatively fall back to the source; the prompt does not bypass this guard. The explicit "I mean" and "correction" cues retain their bounded alignment checks. Anchors stay inside each answer so adjacent corrections can be validated independently.
