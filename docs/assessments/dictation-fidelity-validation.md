...
Preferred-name matching now shares one range-aware implementation for overlap and answer preservation. It joins fragments only across spaces or tabs, preserving punctuation and newline boundaries (`New. York.` cannot become `NewYork.`). Ordinary split names still pass. The completed branch passes **260 Mac tests** and **109 Linux ARM64 tests**.
