...
| `audio.wav` | The PCM received from the selected microphone's input unit, retaining its sample rate and channels before Sotto's downmix/resampling. Current capture uses float32 PCM. WAV storage interleaves channels in little-endian order. This is not a claim of bypassing processing already performed by hardware or macOS. |
