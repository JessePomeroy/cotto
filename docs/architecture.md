...
2. **Start independently.** The controller enters `starting` and shows the HUD before awaiting the microphone. Cached device inventory resolves the input. `AudioCaptureWorker` validates permission/device state and performs input-only AUHAL setup on its serial queue. Model warm-up and detached system-wide Accessibility capture run alongside it. Slow model loading or external AX metadata does not delay the initial indicator.
...
The recorder uses a dedicated input-only AUHAL, following [Apple's device-input guidance](https://developer.apple.com/library/archive/technotes/tn2091/_index.html). Output IO is disabled before the selected microphone is bound. Sotto neither constructs an AVAudioEngine default input/output aggregate nor follows changes to the Mac's default output. It never sets the system default device, hardware sample rate, hog mode, or playback volume. The input's current rate and channels are retained in float32 capture PCM; resampling to 16 kHz happens separately in the writer.

The selected input stays pinned for the take. Preference edits and preferred-device reconnects affect future holds only. Unrelated hardware events validate—but never rebind or restart—the current input; selected-device loss, format changes, or render errors end the take safely. Buffers are bounded by the unit's maximum frame count, release closes admission before teardown, and callback storage outlives disposal. Core Audio status errors are returned as recoverable Swift errors, without the old tap's Objective-C exception path.
