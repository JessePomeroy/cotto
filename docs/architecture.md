...
The recorder sets its own input AudioUnit's device before reading the hardware input format (not the potentially stale client/output format), then pins that route and PCM format for the take. It never changes the system default device or nominal sample rate. A small Objective-C boundary converts AVFAudio exceptions during tap installation, preparation, start, and teardown into recoverable errors; a format change during setup discards the take instead of aborting the app. Resampling to 16 kHz happens in the writer, not on the shared device. Preference edits or a better microphone reconnecting affect future recordings only. Unrelated hardware events keep the current route; loss, format changes, or failed same-route recovery interrupt instead of switching sources mid-sentence.
