...
public actor ConfigurationFile {
    ...
    public func read() -> Result<SottoConfiguration, ConfigurationFileError> {
        result {
    ...
    public func update(from previous: SottoConfiguration, to desired: SottoConfiguration)
        -> Result<SottoConfiguration, ConfigurationFileError> {
        result {
    ...
    private struct Document {
        let configuration: SottoConfiguration
    ...
    private func encoded(_ configuration: SottoConfiguration) throws -> Data {
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys, .withoutEscapingSlashes]
        return try encoder.encode(configuration) + Data("\n".utf8)
    }

    private func decoded(_ data: Data) throws -> SottoConfiguration {
        guard data.count <= Self.maximumFileSize else {
            throw ConfigurationFileError.invalid("The file exceeds the 1 MiB limit.")
        }
        do { return try JSONDecoder().decode(SottoConfiguration.self, from: data) }
