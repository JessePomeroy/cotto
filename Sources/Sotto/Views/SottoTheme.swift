...
enum SottoPalette {
    // Glacier is the dark counterpart to Sotto's unchanged warm light palette.
...
struct SottoSidebarSurface: View {
    ...
    var body: some View {
        if colorScheme == .dark && !reduceTransparency && contrast != .increased {
            SottoSidebarMaterial()
                .overlay(SottoPalette.sidebar.opacity(0.54))
                .overlay {
            ...
            SottoPalette.sidebar
        }
    }
}

private struct SottoSidebarMaterial: NSViewRepresentable {
    ...
    var body: some View {
        if colorScheme == .dark {
            if reduceTransparency || contrast == .increased {
                SottoPalette.surface
            } else {
                SottoPalette.glassTint.opacity(0.71)
                    .overlay {
...
struct SottoMark: View {
    var color: Color = SottoPalette.ink
    var size: CGFloat = 26
...
struct SottoPrimaryButtonStyle: PrimitiveButtonStyle {
    var prominent = true
    ...
    func makeBody(configuration: Configuration) -> some View {
        if prominent {
            Button(configuration)
                ...
                .foregroundStyle(SottoPalette.onAccent)
                .controlSize(.regular)
        } else {
            Button(configuration)
...
struct SottoQuietButtonStyle: PrimitiveButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        Button(configuration)
...
struct SottoSettingsGroup<Content: View>: View {
    @Environment(\.colorSchemeContrast) private var contrast
...
struct SottoFloatingSurface: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        if reduceTransparency || contrast == .increased {
            content
                .background(SottoPalette.surface, in: shape)
                .overlay { shape.strokeBorder(SottoPalette.ink.opacity(0.35), lineWidth: 1) }
            ...
            if colorScheme == .dark {
                ...
                content.background {
                    shape.fill(SottoPalette.glassTint.opacity(0.5))
            ...
            content
                ...
                .background {
                    if colorScheme == .dark { shape.fill(SottoPalette.glassTint.opacity(0.55)) }
                }
                .overlay { shape.strokeBorder(SottoPalette.line, lineWidth: 0.5) }
...
struct KeyCap: View {
    ...
    var body: some View {
        ...
        .foregroundStyle(isPressed ? SottoPalette.accentInk : SottoPalette.muted)
        .padding(.horizontal, 9)
        .frame(height: 27)
        .background {
            RoundedRectangle(cornerRadius: 5, style: .continuous)
                .fill(isPressed ? SottoPalette.tint : SottoPalette.surface)
                .overlay {
                    RoundedRectangle(cornerRadius: 5, style: .continuous)
                        .strokeBorder(isPressed ? SottoPalette.accentInk : SottoPalette.line, lineWidth: 0.5)
                }
        }
        .accessibilityElement(children: .ignore)
    ...
    var body: some View {
        ...
        ZStack(alignment: .top) {
            ...
            Text(key.symbol)
                ...
                .background(isPressed ? SottoPalette.tint : SottoPalette.surface, in: shape)
                .overlay { shape.strokeBorder(SottoPalette.line, lineWidth: contrast == .increased ? 1.5 : 0.7) }
...
struct LiveWaveform: View {
    var levels: [Float]
    var color: Color = SottoPalette.accent
...
struct InlineNotice: View {
    ...
    var body: some View {
        ...
        .foregroundStyle(isError ? SottoPalette.warning : SottoPalette.muted)
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(12)
        .background((isError ? SottoPalette.warning : SottoPalette.muted).opacity(0.06))
