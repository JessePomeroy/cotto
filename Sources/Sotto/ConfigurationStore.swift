...
final class ConfigurationStore: ObservableObject {
    @Published private(set) var configuration: SottoConfiguration
    ...
    private struct Edit {
        var before: SottoConfiguration
        var after: SottoConfiguration
    }
    ...
    func update(_ edit: (inout SottoConfiguration) -> Void) {
    ...
    private func accept(_ result: Result<SottoConfiguration, ConfigurationFileError>, publish: Bool) {
        switch result {
...
private final class ConfigurationWatcher: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "local.sotto.configuration-watch", qos: .utility)
