...
actor HelperProcess {
    ...
    func request(_ data: Data, id: String, timeout: Double,
        ...
        return try await withTaskCancellationHandler {
            ...
            return try await withCheckedThrowingContinuation { continuation in
                ...
                DispatchQueue(label: "app.sotto.server.helper-input").async {
    ...
    private func readLines(_ handle: FileHandle, generation current: UUID) {
        let limit = lineLimit
        DispatchQueue(label: "app.sotto.server.helper-output", qos: .userInitiated).async { [weak self] in
    ...
    private func drainDiagnostics(_ handle: FileHandle) {
        DispatchQueue(label: "app.sotto.server.helper-diagnostics", qos: .utility).async {
            defer { try? handle.close() }
