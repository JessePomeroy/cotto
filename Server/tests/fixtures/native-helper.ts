...
for await (const chunk of process.stdin) {
  ...
  while ((index = buffer.indexOf("\n")) !== -1) {
    ...
    const request = JSON.parse(line) as { id: string; type: string; vocabularyTerms?: string[]; systemPrompt?: string };
    if (mode === "no-result") continue;
    if (mode === "ignore-term") {
      setTimeout(() => emit({ type: "unexpected" }), 100);
      continue;
    }
    if (mode === "oversized") { process.stdout.write("x".repeat(70_000)); continue; }
    if (mode === "invalid-json") { process.stdout.write("not json\n"); continue; }
    if (mode === "unknown-event") { emit({ type: "unexpected" }); continue; }
    ...
    emit({ type: "result", id: "stale-request", text: "Wrong request.", duration: 2, elapsed: 0.1, language: "en" });
    ...
    const result = {
      type: "result", id: request.id, text: mode === "invalid-result" ? "\0bad" : "Hello world.",
      duration: 2, elapsed: 0.1, language: "en",
      ...(request.type === "transcribe" ? {
        includedTerms: mode === "invalid-hints" ? ["invented"] : terms,
        omittedTerms: [], tokenCount: 1, tokenBudget: 223,
      } : {}),
    };
    if (mode === "split-json") {
