...
describe("spoken-list grammar", () => {
  const formattedExamples = [
    ...
    [
      "Make a list. Twenty-one, apples. Number thirty two, bananas. Item one hundred and three, oranges.",
      "21. apples\n32. bananas\n103. oranges",
    ],
    ...
    [
      ...
      "1. buy 3.5 litres for $12.50\n2. call Dr. Green in the U.S.\n3. explain the problem. Include screenshots!\n4. first paragraph\n\nsecond paragraph",
    ],
    [
      "Please send the report tomorrow. Continue the list. Three, oranges.",
      "Please send the report tomorrow.\n\n3. oranges",
    ],
    [
      "Make a list. One, my dictation was broken. Two, the screenshot was wrong.",
      "1. my dictation was broken\n2. the screenshot was wrong",
    ],
    ["Start a bulleted list. Bullet point apples.", "- apples"],
    [
      "Start a list. Number five is book the room. 3 is pick up the keys. Item two is send the invitation.",
  ...
  const ordinaryExamples = [
    ...
    "Call at four.",
    "One thing matters.",
    "First we test. Second we ship.",
    "First we test.\n\nSecond we ship.",
    "We need 3.5 litres and $12.50 for lunch.",
    "Meet at 4:30 on 2026-09-03.",
    "Use 1/2 cup of sugar and 3/4 cup of flour.",
    "One, two, three.",
    "One, 2, three.",
    "2026. Revenue increased. 2027. We expect growth.",
    "The end of the list is missing.",
    "Make a list of my expenses for tomorrow.",
    "Sorry, I wanted that screenshot. My voice to text was just ruined.",
    "Item 123 is missing.",
    "Number one is our priority.",
    "New item added to the cart.",
    "Bullet point formatting is broken.",
    "Next item arrives tomorrow.",
    "Next bullet hits the target.",
    "One is enough. Three is excessive.",
    "This one is ready. That one is missing.",
    "I have a list. This one is ready. That one is missing.",
    "I have a list. One is missing.",
  ...
  test("announced copular markers keep repeats, skips and decreases", () => {
    const source =
      "Okay, I have a list of things to do today. One is I need to book the room Three is I need to pick up the keys. Two is I need to send the invitation. Four is I need to check the meeting room.";
    expect(formatSpokenList(source).text).toBe(
      "Okay, I have a list of things to do today.\n\n1. I need to book the room\n3. I need to pick up the keys\n2. I need to send the invitation\n4. I need to check the meeting room",
    );
    for (const introduction of [
      ...
      const result = formatSpokenList(
        `${introduction} Five is book the room. Three is check that this one is available. Three is call the host.`,
      );
      expect(
  ...
  test("explicit and implicit continuations advance only emitted items", () => {
    ...
    const resumed = formatSpokenList(
      "Continue the list. Number four, a trip to the beach. Item seven, more syrup.",
      next.context,
    );
    ...
    expect(formatSpokenList("Next item, one, two, three.", numbered(4)).text).toBe(
      "4. one, two, three",
    );
  });
  ...
  test("resume removes only explicit tool chatter", () => {
    const prelude =
      "Sorry, I wanted that screenshot. Go ahead. My voice to text was just ruined. Let me go back to where I was with that list.";
    const result = formatSpokenList(
      `${prelude} Three, oranges. Four, a trip to the beach. Seven, more syrup. That's the end of the list.`,
    );
    ...
    for (const kept of [
      ...
      expect(formatSpokenList(`${kept} Continue the list. Three, oranges.`).text).toBe(
        `${kept}\n\n3. oranges`,
      );
    }
  });
  ...
  test("controls and silence preserve continuation without adding content", () => {
    ...
    expect(
      formatSpokenList("Start a new list. One, pears.", numbered(8)).continuesPreviousList,
    ).toBe(false);
    expect(
      formatSpokenList("Start a list. Next item. End list.").formattingRejectionReason,
    ).toBeUndefined();
  });

  test("bullets continue and explicit end returns to prose", () => {
    const first = formatSpokenList(
      "Start a bulleted list. Bullet point, apples. Next bullet, bananas.",
    );
    ...
    const prose = formatSpokenList(
      "Three, oranges. End of list. Please send the receipt tomorrow.",
      numbered(3),
    );
    ...
    expect(
      formatSpokenList("Start a list. One, first paragraph\n\nsecond paragraph.").endsWithList,
    ).toBe(true);
  });
  ...
  test("ASR missing punctuation needs an anchored increasing series", () => {
    for (const [source, expected, next] of [
      [
        "5, Apples 6, Bananas 7, Oranges 8, Pears",
        "5. Apples\n6. Bananas\n7. Oranges\n8. Pears",
        9,
      ],
  ...
  test("diagnostic control spans use UTF-16 and exclude numbers and item text", () => {
    ...
    expect(
      result.consumedControls.map(({ location, length }) =>
        source.slice(location, location + length),
      ),
    ).toEqual(["Continue the list.", "End list."]);
    expect(result.consumedControls[0]?.location).toBe(source.indexOf("Continue"));
    expect(replaceFormattedText(result, "Updated").consumedControls).toEqual(
      result.consumedControls,
    );
    expect(formatSpokenList("Start a list. One, café. Two, 👨‍👩‍👧‍👦 family.").text).toBe(
      "1. café\n2. 👨‍👩‍👧‍👦 family",
    );
  });
});
