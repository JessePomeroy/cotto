...
import { NativeInference, createInferenceConfiguration, InferenceError, type InferenceConfiguration } from "../src/inference/native-inference";
...
async function fixture(mode = "valid", overrides: Partial<InferenceConfiguration> = {}, pinned = false) {
  ...
  const configuration = createInferenceConfiguration({
    ...
    speechLoadTimeout: 3, speechTimeout: 3, proofLoadTimeout: 3, proofTimeout: 3,
    ...overrides,
  });
  const digest = createHash("sha256").update("fixture model").digest("hex");
  const inference = new NativeInference(configuration, pinned ? { speech: { sha256: digest } } : {});
...
describe("native inference subprocess protocol", () => {
  test("warm helpers serve both requests, clamp matching progress, and survive idle cancellation", async () => {
    ...
    expect(await inference.readiness()).toMatchObject({ available: true, speechLoaded: true, proofLoaded: true });
    const progress: number[] = [];
    const speech = await inference.transcribe(model, "en", ["auth"], value => progress.push(value));
    expect(speech).toMatchObject({ text: "Hello world.", audioSeconds: 2, processingSeconds: 0.1, language: "en", engineVersion: "fixture-1" });
    expect(speech.hints).toEqual({ includedTerms: ["auth"], omittedTerms: [], tokenCount: 1, tokenBudget: 223 });
    expect(progress).toEqual([0, 0.5, 1]);
    expect(await inference.correct(speech.text, ["Sotto"], "en", "Keep punctuation.")).toMatchObject({ text: "Hello world.", engineVersion: "fixture-1" });
  });
  ...
  test("invalid vocabulary fails before starting a helper", async () => {
    ...
    for (const terms of [["auth", "auth"], [" auth"], ["auth "], [" "], ["auth\u00a0"], ["auth\nterm"], ["auth\u200bterm"], ["x".repeat(16_385)]]) {
      await expect(inference.transcribe(model, "en", terms)).rejects.toMatchObject({ code: "invalidRequest" });
    }
  ...
  test("malformed vocabulary diagnostics and result content invalidate the helper", async () => {
    for (const mode of ["invalid-hints", "invalid-result"]) {
      const { inference, model } = await fixture(mode);
      await expect(inference.transcribe(model, "en", ["auth"])).rejects.toMatchObject({ code: "invalidResponse" });
      expect((await inference.readiness(false)).speechLoaded).toBe(false);
    }
  });
  ...
  test("loading and inference deadlines resolve waiters and reset processes", async () => {
    ...
    await expect(running.inference.transcribe(running.model, "en", [])).rejects.toMatchObject({ code: "timeout" });
  ...
  test("stale output cannot reset replacements and ignored SIGTERM escalates to SIGKILL", async () => {
    ...
    const helper = new HelperProcess({ name: "Fixture", executable, arguments: ["--model", model], requiredFiles: [model], loadTimeout: 3, lineLimit: 65_536 });
    try {
      await helper.ensureLoaded();
      const oldPID = Number(helper.snapshot().engineVersion?.slice(4));
      expect(oldPID).toBeGreaterThan(0);
      const request = helper.request({ type: "transcribe", id: "first" }, "first", 3).catch(error => error);
      await Bun.sleep(20);
      helper.cancel();
    ...
    } finally { await helper.shutdown(); }
  }, 7000);

  test("an immediately exiting parent leaves no current or previously cancelled native helpers", async () => {
    const { executable, model } = await fixture("ignore-term");
    const parent = Bun.spawn([process.execPath, new URL("./fixtures/native-shutdown-parent.ts", import.meta.url).pathname, executable, model], {
    ...
    try {
      ...
      if (!helperPIDs.length) {
        try { helperPIDs = (await readFile(`${model}.pids`, "utf8")).trim().split("\n").map(Number); }
        catch {}
      }
      for (const pid of helperPIDs) { try { process.kill(pid, "SIGKILL"); } catch {} }
    }
  }, 5000);

  test("oversized unterminated lines are bounded before JSON decoding", async () => {
    const { inference } = await fixture("oversized");
    await expect(inference.correct("Hello.", [], "en", "Keep punctuation.")).rejects.toMatchObject({ code: "unavailable", message: expect.stringContaining("size limit") });
  });

  test("invalid JSON and unknown events reset the helper", async () => {
    for (const mode of ["invalid-json", "unknown-event"]) {
      const { inference, model } = await fixture(mode);
      await expect(inference.transcribe(model, "en", [])).rejects.toMatchObject({ code: "invalidResponse" });
    }
  });

  test("missing proof assets do not block speech-only readiness", async () => {
    const { inference, configuration } = await fixture("no-ready", { proofModel: "/nonexistent/sotto-proof-model" });
...
describe("model integrity", () => {
  test("digest cache is invalidated by same-size edits and inode replacement", async () => {
    ...
    expect((await inference.readiness(false)).available).toBe(false);
    await expect(inference.warmUp(false)).rejects.toMatchObject({ code: "unavailable", message: expect.stringContaining("SHA-256") });
  ...
  test("models must be regular files and cannot be symlinks", async () => {
    ...
    await expect(verifier.verify(directory, { sha256: digest })).rejects.toMatchObject({ code: "unavailable" });
    await expect(verifier.verify(model, { bytes: 1, sha256: digest })).rejects.toMatchObject({ code: "unavailable", message: expect.stringContaining("incorrect size") });
  });

  test("concurrent verification shares a hash without losing successful callers", async () => {
    const { model, digest } = await fixture();
    const verifier = new ModelVerifier();
    expect(await Promise.all([verifier.verify(model, { sha256: digest }), verifier.verify(model, { sha256: digest })])).toEqual([digest, digest]);
  });

  test("configuration clamps invalid deadlines and thread counts", () => {
    const configuration = createInferenceConfiguration({ speechHelper: "a", speechModel: "b", vadModel: "c", proofHelper: "d", proofModel: "e", threads: 100, speechTimeout: NaN, proofTimeout: -1, speechLoadTimeout: 3601 });
    expect(configuration).toMatchObject({ threads: 32, speechTimeout: 180, proofTimeout: 18, speechLoadTimeout: 120, proofLoadTimeout: 30 });
  });
});
