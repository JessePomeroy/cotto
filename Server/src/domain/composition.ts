...
export function composeDictation(formatted: FormattedDictation, previous?: DictationContinuation) {
  ...
  if (body.length === 0) {
    if (!formatted.isControlOnly) return { insertion: "", preview: previous?.preview ?? "", continuation: previous };
    if (formatted.endedList && formatted.context === undefined) {
      ...
      const continuation: DictationContinuation | undefined = needsParagraph ? { preview, boundary: "paragraph" } : undefined;
    ...
    const needsParagraph = previous?.boundary === "paragraph" || (previous !== undefined && previous.preview.length > 0);
  ...
  if (previous !== undefined) {
    if (continuing) separator = previous.boundary === "paragraph" ? "\n\n" : previous.boundary === "line" ? "\n" : "";
    else if (previous.boundary === "paragraph" || previous.list !== undefined ||
             (previous.preview.length > 0 && (formatted.containsList || formatted.context !== undefined))) separator = "\n\n";
  }
  const finishingPreviousList = formatted.endedList && previous?.list !== undefined;
  const keepPreview = continuing || ((previous?.boundary === "paragraph" || finishingPreviousList) && !formatted.containsList);
  const preview = keepPreview && previous !== undefined && previous.preview.length > 0 ? previous.preview + separator + body : body;
  const suffix = formatted.endsWithList || formatted.context !== undefined ? "" : " ";
  const boundary = formatted.context !== undefined ? "line" : formatted.endsWithList ? "paragraph" : "none";
  const continuation: DictationContinuation = { list: formatted.context, preview, boundary };
...
export class DictationContinuationMemory<Anchor> {
  ...
  forget(anchor: Anchor) { this.entries = this.entries.filter((entry) => entry.anchor !== anchor); }
  removeAll() { this.entries = []; }
  private expire(now: number) { this.entries = this.entries.filter((entry) => now >= entry.timestamp && now - entry.timestamp < this.lifetime); }
}
