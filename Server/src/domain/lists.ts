...
type Action = { kind: "item"; marker: Marker } | { kind: "start" | "resume"; style?: Style } | { kind: "end" };
type Evidence = "explicit" | "numeric" | "series" | "unanchoredSeries" | "inlineSeries" | "copularSeries" | "contextOnly";
...
const integer = (value: string) => /^[+]?[0-9]+$/.test(value) && Number.isSafeInteger(Number(value)) ? Number(value) : undefined;
const contains = (range: Range, position: number) => position >= range.start && position < range.end;
const boundary = (token: Token) => [".", "!", "?", ";", ":", "\n"].includes(token.value)
  || (token.value.endsWith(".") && !["dr.", "mr.", "mrs.", "ms.", "st."].includes(token.value));
const abbreviations = new Set(["mr", "mrs", "ms", "dr", "jr", "sr", "st", "vs", "etc", "e.g", "i.e"]);
...
function tokenize(text: string): Token[] {
  const chars = Array.from(segmenter.segment(text), ({ segment, index }) => ({ value: segment, index }));
  const result: Token[] = [];
  let cursor = 0;
  while (cursor < chars.length) {
    ...
    if (content(first)) {
      while (cursor < chars.length) {
        ...
        const numericJoin = [".", ",", ":", "/", "-"].includes(current) && numeric(previous) && numeric(next);
      ...
      if (chars[cursor]?.value === ".") {
        ...
        const initials = word.includes(".") && letters.length > 0 && letters.every((value) => value.toUpperCase() === value && value.toLowerCase() !== value);
    ...
    result.push({ value: text.slice(range.start, range.end).toLowerCase().replaceAll("’", "'"), range });
  }
  return result;
}

const smallNumbers = new Map([
  ...["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen", "seventeen", "eighteen", "nineteen"].map((word, number) => [word, number] as const),
  ...["zeroth", "first", "second", "third", "fourth", "fifth", "sixth", "seventh", "eighth", "ninth", "tenth", "eleventh", "twelfth", "thirteenth", "fourteenth", "fifteenth", "sixteenth", "seventeenth", "eighteenth", "nineteenth"].map((word, number) => [word, number] as const),
]);
...
function englishNumber(words: string[]): number | undefined {
  ...
  if (words.length === 2 && tensValue !== undefined && units !== undefined && units >= 1 && units <= 9) return tensValue + units;
  const leading = smallNumbers.get(first);
  if (leading !== undefined && leading >= 1 && leading <= 9 && ["hundred", "hundredth", "thousand", "thousandth"].includes(words[1]!)) {
...
function number(tokens: Token[], start: number) {
  ...
  for (let end = start; end < Math.min(tokens.length, start + 6); end += 1) {
    ...
    if (!next.length || !next.every((word) => smallNumbers.has(word) || tens.has(word) || ["hundred", "hundredth", "thousand", "thousandth", "and"].includes(word))) break;
    words.push(...next);
...
function marker(tokens: Token[], start: number): Match | undefined {
  const has = (words: string[]) => words.every((word, index) => tokens[start + index]?.value === word);
  const endAfterSeparator = (index: number) => tokens[index] && [",", ".", ":", ";", ")", "-", "–", "—"].includes(tokens[index]!.value) ? index + 1 : index;
  const phraseMarker = (end: number, marker: Marker): Match => {
    const separatedEnd = endAfterSeparator(end);
    return { end: separatedEnd, action: { kind: "item", marker }, evidence: separatedEnd !== end || end === tokens.length ? "explicit" : "contextOnly" };
  };
  for (const words of [["next", "bullet", "point"], ["next", "bullet"], ["bullet", "point"], ["new", "bullet"]]) {
    if (has(words)) return phraseMarker(start + words.length, { kind: "bullet" });
  }
  for (const words of [["next", "item"], ["new", "item"]]) {
    if (has(words)) return phraseMarker(start + words.length, { kind: "next" });
  }
  if (has(["next"]) && [",", ":"].includes(tokens[start + 1]?.value ?? "")) return { end: start + 2, action: { kind: "item", marker: { kind: "next" } }, evidence: "contextOnly" };
  if (["-", "•", "–"].includes(tokens[start]!.value) && tokens[start + 1]
    && tokens[start]!.range.end < tokens[start + 1]!.range.start && (start === 0 || tokens[start - 1]!.value === "\n")) return { end: start + 1, action: { kind: "item", marker: { kind: "bullet" } }, evidence: "explicit" };
  ...
  if (!parenthesized && parsed.value < 1_000 && tokens[parsed.end]?.value === "is") return { end: parsed.end + 1, action, evidence: "copularSeries" };
  ...
  const explicit = parenthesized || (integer(tokens[numberStart]!.value) !== undefined && [".", ")", ":"].includes(separator));
...
function listDescriptor(tokens: Token[], start: number) {
  let end = start;
  const descriptors = new Set(["a", "an", "the", "that", "this", "my", "our", "new", "previous", "same", "numbered", "ordered", "bulleted", "unordered", "bullet", "point"]);
  while (end < tokens.length && descriptors.has(tokens[end]!.value)) end += 1;
  const words = tokens.slice(start, end).map((token) => token.value);
  if (tokens[end]?.value === "list" || (tokens[end]?.value === "points" && words.includes("bullet"))) return { end: end + 1, style: styleHint(words) };
  return undefined;
}

function introducesList(tokens: Token[], start: number) {
  return [["i", "have"], ["we", "have"], ["here", "is"], ["here's"], ["this", "is"]].some((prefix) => prefix.every((word, index) => tokens[start + index]?.value === word) && listDescriptor(tokens, start + prefix.length) !== undefined);
}
...
function directive(tokens: Token[], start: number): Match | undefined {
  ...
  else if (tokens[index]?.value === "let" && ["me", "us"].includes(tokens[index + 1]?.value ?? "")) index += 2;
  if (!tokens[index]) return undefined;
  const finish = (end: number, action: Action): Match | undefined => end === tokens.length ? { end, action, evidence: "explicit" }
    : [".", ",", ":", ";", "!", "?", "\n"].includes(tokens[end]?.value ?? "") ? { end: end + 1, action, evidence: "explicit" } : undefined;
  ...
  if (["go", "get"].includes(tokens[index]!.value) && tokens[index + 1]?.value === "back") index += 1;
  if (tokens[index]!.value === "back") {
    const allowed = new Set(["to", "where", "i", "was", "with", "on", "the", "that", "this", "my", "our", "previous", "same", "numbered", "bulleted", "bullet", "point"]);
    let end = index + 1;
    while (end < Math.min(tokens.length, index + 16) && allowed.has(tokens[end]!.value)) end += 1;
    if (tokens[end]?.value === "list") return finish(end + 1, { kind: "resume", style: styleHint(tokens.slice(index, end + 1).map((token) => token.value)) });
  }
  return undefined;
}
...
function scan(text: string): Event[] {
  ...
  while (index < tokens.length) {
    ...
    if (index === itemBodyStart && !(index > 0 && boundary(tokens[index - 1]!))) { index += 1; continue; }
    const command = atBoundary ? directive(tokens, index) : undefined;
    if (command) {
      events.push({ range: { start: tokens[index]!.range.start, end: tokens[command.end - 1]!.range.end }, action: command.action, evidence: command.evidence });
      index = command.end;
      afterDirective = true;
    ...
    if (match.evidence === "copularSeries" && (!hasListIntroduction || (index > 0 && ["this", "that", "the", "each", "every", "only", "which"].includes(tokens[index - 1]!.value)))) { index += 1; continue; }
    if (match.evidence === "numeric" && !hasListEvidence && index > 0 && tokens[index - 1]!.value !== "\n") match = { ...match, evidence: "unanchoredSeries" };
    if (!atBoundary && match.evidence !== "copularSeries") {
      if (match.action.kind !== "item" || match.action.marker.kind !== "number" || integer(tokens[index]!.value) === undefined || tokens[index + 1]?.value !== ",") { index += 1; continue; }
    ...
    if (afterComma && match.evidence !== "copularSeries") {
      ...
      if ((!hasListEvidence && match.evidence !== "explicit") || (previousWasNumber && match.evidence !== "explicit")) { index += 1; continue; }
    }
    events.push({ range: { start: tokens[index]!.range.start, end: tokens[match.end - 1]!.range.end }, action: match.action, evidence: match.evidence });
    index = match.end;
    itemBodyStart = index;
...
const contentTokens = (text: string) => tokenize(text).map((token) => token.value).filter(content);
...
function inferredMarkers(events: Event[], text: string) {
  ...
  let series: number[] = [];
  const body = (index: number) => text.slice(events[index]!.range.end, events[index + 1]?.range.start ?? text.length);
  const finishSeries = () => {
    ...
    if (pending.some((index) => events[index]!.evidence === "inlineSeries")) {
      ...
      const numbers = pending.flatMap((index) => {
        ...
        return action.kind === "item" && action.marker.kind === "number" ? [action.marker.number] : [];
      });
      if (!numbers.every((value, index) => index === 0 || numbers[index - 1]! < value)) return;
      const hedges = new Set(["and", "or", "maybe", "perhaps", "possibly", "about", "around", "roughly", "approximately"]);
      if (!pending.every((index) => { const words = contentTokens(body(index)); return words.some(letter) && !!words[0] && !hedges.has(words[0]); })) return;
    }
    for (const index of pending) accepted.add(index);
  };
  for (const [index, event] of events.entries()) {
    if (event.action.kind !== "item") { finishSeries(); continue; }
    if (event.action.marker.kind === "number" && event.evidence !== "contextOnly" && content(body(index))) series.push(index);
    else finishSeries();
  }
  finishSeries();
  return accepted;
...
function itemText(text: string) {
  ...
  if (tokens.at(-1)?.value === "." && tokens.slice(0, -1).every((token) => ![".", "!", "?"].includes(token.value))) return value.slice(0, -1).trim();
  return value;
}

function isMeta(text: string) {
  const words = tokenize(text).map((token) => token.value).filter(letter);
  const sentence = words.join(" ");
  if (["sorry", "go ahead", "hold on", "hang on", "one moment", "wait a second", "just a second", "let me try again", "let's try again", "okay", "ok", "all right"].includes(sentence)) return true;
  const speechTools = ["dictation", "transcription", "voice to text", "voice-to-text", "speech to text", "speech-to-text"];
  const problems = new Set(["ruined", "broken", "wrong", "messed", "stopped", "lost", "failed", "restarting", "restart", "redo", "again"]);
  const status = speechTools.map((tool) => { const prefix = `my ${tool} `; return sentence.startsWith(prefix) ? sentence.slice(prefix.length) : undefined; }).find((value) => value !== undefined);
  const statusVerbs = new Set(["is", "was", "has", "had", "got", "went", "just", "keeps", "stopped", "failed", "broke"]);
  if (status !== undefined && statusVerbs.has(status.split(" ")[0] ?? "") && !text.includes("?") && !words.includes("please") && words.some((word) => problems.has(word))) return true;
  return (sentence.includes("screenshot") || sentence.includes("screen shot")) && ["sorry", "i"].includes(words[0] ?? "") && words.some((word) => ["wanted", "wrong", "meant"].includes(word));
}
...
function removing(removed: Range[], text: string, range: Range) {
  let kept = "";
  let start = range.start;
  for (const removal of removed) { kept += text.slice(start, removal.start); start = removal.end; }
  return kept + text.slice(start, range.end);
}

const numberToken = (value: number) => `#list-number:${value}`;
function sourceContentTokens(text: string, controls: Range[], numberMarkers: { range: Range; number: number }[]) {
  const result: string[] = [];
  for (const token of tokenize(text)) {
    ...
    if (marker) { if (marker.range.start === token.range.start) result.push(numberToken(marker.number)); continue; }
...
export function formatSpokenList(text: string, initialContext?: SpokenListContext): FormattedDictation {
  const empty = (value: string, reason?: string): FormattedDictation => ({ text: value, context: initialContext, containsList: false, endsWithList: false, continuesPreviousList: false, endedList: false, isControlOnly: false, consumedControls: [], formattingRejectionReason: reason });
  ...
  const flush = () => {
    ...
    if (!value) return;
    if (!context) { pieces.push({ text: value, isList: false }); emittedContent.push(...contentTokens(value)); return; }
    const item = itemText(value);
    if (!item) return;
    if (!pieces.length && usesOriginalContext && initialContext?.style === context.style) continuesPreviousList = true;
    if (context.style === "numbered") {
      ...
      context = { style: "numbered", nextNumber: Math.min(Number.MAX_SAFE_INTEGER, context.nextNumber + 1) };
  ...
  for (const [index, event] of events.entries()) {
    ...
    if (action.kind === "item") {
      const followingBody = text.slice(event.range.end, events[index + 1]?.range.start ?? text.length);
      if (action.marker.kind === "number" && !content(followingBody)) { body += text.slice(event.range.start, event.range.end); continue; }
      if (event.evidence === "series" && !containsNonCountingContent(followingBody)) { body += text.slice(event.range.start, event.range.end); continue; }
      const contextSupportsMarker = context !== undefined && !["inlineSeries", "unanchoredSeries", "copularSeries"].includes(event.evidence)
        && !(event.evidence === "series" && containsInlineCandidates && !inferred.has(index));
      if (!(contextSupportsMarker || event.evidence === "explicit" || event.evidence === "numeric" || inferred.has(index))) { body += text.slice(event.range.start, event.range.end); continue; }
      flush();
      ...
      } else if (action.marker.kind === "bullet") { context = { style: "bulleted", nextNumber: 1 }; controls.push(event.range); }
      else { context ??= { style: "numbered", nextNumber: 1 }; controls.push(event.range); }
      ...
      flush();
      if (action.style !== undefined && action.style !== context?.style) { context = { style: action.style, nextNumber: 1 }; usesOriginalContext = false; }
  ...
  flush();
  if (!consumedControl && initialContext === undefined) return { ...empty(text), context: undefined };
  const output = pieces.map((piece, index) => `${index === 0 ? "" : pieces[index - 1]!.isList && piece.isList ? "\n" : "\n\n"}${piece.text}`).join("");
  if (!output && consumedControl && usesOriginalContext && context && context.style === initialContext?.style) continuesPreviousList = true;
  const sourceContent = sourceContentTokens(text, controls, numberMarkers);
  if (sourceContent.length !== emittedContent.length || sourceContent.some((token, index) => token !== emittedContent[index])) return empty(text, "List formatting would change dictated content; kept the original transcript.");
  return { text: output, context, containsList, endsWithList: pieces.at(-1)?.isList === true, continuesPreviousList, endedList, isControlOnly: consumedControl && !output, consumedControls: controls.map(({ start, end }) => ({ location: start, length: end - start })), formattingRejectionReason: undefined };
}

export function replaceFormattedText(formatted: FormattedDictation, text: string): FormattedDictation {
