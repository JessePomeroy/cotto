...
final class BoundedLineReader: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "app.sotto.text-input", qos: .userInitiated)
...
/// The helper must not outlive Sotto, even if another process retains stdin.
final class ParentWatcher {
    ...
    init() {
        ...
        let queue = DispatchQueue(label: "app.sotto.text-parent", qos: .utility)
